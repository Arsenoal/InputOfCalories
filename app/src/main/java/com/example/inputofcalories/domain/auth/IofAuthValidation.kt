...
class IofAuthValidation: AuthValidationUseCase {
    ...
    override suspend fun checkPasswordsMatch(p1: String, p2: String)
            = (p1 == p2)

    override suspend fun checkRegistrationFieldsAreFilled(userRegistrationParams: UserRegistrationParams)
            = with(userRegistrationParams) {
        name.isNotBlank() && email.isNotBlank() && password.isNotBlank() && repeatPassword.isNotBlank()
    }

    override suspend fun checkSignInFieldsAreFilled(userSignInParams: UserSignInParams)
            = with(userSignInParams) {
        email.isNotBlank() && password.isNotBlank()
    }

    override suspend fun checkEmailFormat(email: String)
